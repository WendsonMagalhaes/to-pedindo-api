...
class PedidoController {
    async create(req, res) {
        try {
            const result = await PedidoService.create(req.body);
    ...
    async findById(req, res) {
        try {
            const result = await PedidoService.findById(req.params.id);
    ...
    async update(req, res) {
        try {
            const result = await PedidoService.update(req.params.id, req.body);
            res.json(result);
        } catch (err) {
            res.status(500).json({ error: err.message });
        }
